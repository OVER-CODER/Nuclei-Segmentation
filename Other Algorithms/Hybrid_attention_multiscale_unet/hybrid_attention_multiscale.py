...
def predict_unet(model, test_loader):
    ...
    with torch.no_grad():
        for img_tensor, mask_tensor, original_img, original_mask, filename in tqdm(test_loader, desc="Predicting"):
            ...
            predictions.append((output[0], mask_tensor.cpu().numpy()))
...
def evaluate_predictions(predictions, gt_masks):
    ...
    for (pred_mask, gt_mask_tensor), filename in zip(predictions, gt_masks.keys()):
        gt_mask = gt_masks.get(filename)
        if gt_mask is None:
            print(f"Ground truth for {filename} not found.")
            continue

        pred_mask_binary = (pred_mask > 0.5).flatten().astype(np.uint8)
        gt_mask_array = gt_mask_tensor.cpu().numpy()
...
if __name__ == '__main__':
    ...
    hmsam_unet_model = train_unet(hmsam_unet_model, train_loader, num_epochs=20, lr=0.001)
